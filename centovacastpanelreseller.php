 ...
 */
class Centovacastpanelreseller extends Module
{
    /**
    ...
    public function __construct()
    {
        ...
        Language::loadLang('centovacastpanelreseller', null, dirname(__FILE__) . DS . 'language' . DS);
    ...
    {
        return [
            'tabStats' => Language::_('Centovacastpanelreseller.tab_stats', true)
        ];
    }
    ...
    {
        return [
            'tabClientActions' => Language::_('Centovacastpanelreseller.tab_client_actions', true),
            'tabClientStats' => Language::_('Centovacastpanelreseller.tab_stats', true)
        ];
    }
    ...
    {
        return ['first' => Language::_('Centovacastpanelreseller.order_options.first', true)];
    }
    ...
    public function manageModule($module, array &$vars)
    {
        ...
        $this->view->setDefaultView('components' . DS . 'modules' . DS . 'centovacastpanelreseller' . DS);
    ...
    public function getAdminAddFields($package, $vars = null)
    {
		...
		$radiousername = $fields->label(Language::_('Centovacastpanelreseller.service_field.radiousername', true), 'radiousername');
		...
		$radiopassword = $fields->label(Language::_('Centovacastpanelreseller.service_field.radiopassword', true), 'radiopassword');
    ...
    public function getAdminEditFields($package, $vars = null)
    {
        ...
        $radiousername->attach($fields->tooltip(Language::_('Centovacastpanelreseller.service_field.radiousername.tooltip', true)));
        $fields->setField($radiousername);

        $radiopassword = $fields->label(
            Language::_('Centovacastpanelreseller.service_field.radiopassword', true),
            'radiopassword'
        );
        $radiopassword->attach($fields->fieldPassword('radiopassword', ['id' => 'radiopassword']));
        $radiopassword->attach($fields->tooltip(Language::_('Centovacastpanelreseller.service_field.radiopassword.tooltip', true)));
        $fields->setField($radiopassword);
        ...
        $rules = [
            'radiousername' => [
                'empty' => [
                    ...
                    'rule' => 'isEmpty',
                    'negate' => true,
                    'message' => Language::_('Centovacastpanelreseller.!error.radiousername.empty', true)
                ]
            ],
        ];
    ...
    public function addService(
        ...
        if (!$row) {
            $this->Input->setErrors(
                ['module_row' => ['missing' => Language::_('Centovacastpanelreseller.!error.module_row.missing', true)]]
            );
		...
		$params['maxbitrate'] = $vars['configoptions']['maxbitrate'];
		$params['resellerusers'] = $vars['configoptions']['resellerusers'];
		$params['maxclients'] = $vars['configoptions']['maxclients'];
		$params['transferlimit'] = $vars['configoptions']['transferlimit'];
		$params['resellerbandwidth'] = $vars['configoptions']['resellerbandwidth'];
    ...
    public function suspendService($package, $service, $parent_package = null, $parent_service = null)
    {
        if (($row = $this->getModuleRow())) {
            ...
            if (!$response['status']) {
                $this->Input->setErrors(['api' => ['error' => '[Suspend] '.Language::_('Centovacastpanelreseller.!error.api.internal', true)]]);
            }
        }
    ...
    public function unsuspendService($package, $service, $parent_package = null, $parent_service = null)
    {
        if (($row = $this->getModuleRow())) {
            ...
            // If the action fails then set an error
            if (!$response['status']) {
                $this->Input->setErrors(['api' => ['error' => '[Unsuspend] '.Language::_('Centovacastpanelreseller.!error.api.internal', true)]]);
            }
        }
    ...
    public function cancelService($package, $service, $parent_package = null, $parent_service = null)
    {
        if (($row = $this->getModuleRow())) {
            ...
            // If the action fails then set an error
            if (!$response['status']) {
                $this->Input->setErrors(['api' => ['error' => '[Cancel] '.Language::_('Centovacastpanelreseller.!error.api.internal', true)]]);
            }
        }
    ...
    private function parseResponse($response)
    {
        ...
        if ($response['status'] == false) {
            $this->Input->setErrors(['api' => ['error' => '[Parse] '.Language::_('Centovacastpanelreseller.!error.api.internal', true)]]);
            $success = false;
        }
    ...
    {
        Loader::load(dirname(__FILE__) . DS . 'api' . DS . 'centovacastpanelreseller_api.php');

        return new CentovacastpanelresellerApi($adminapikey, $ipaddress, $usessl, $adminusername, $useproxy);
    }
        ...
        $rules = [
		    'server_name' => [
                'valid' => [
                    'rule' => 'isEmpty',
                    'negate' => true,
                    'message' => Language::_('Centovacastpanelreseller.!error.server_name_valid', true)
                ]
            ],
			'ipaddress' => [
                'valid' => [
                    'rule' => [[$this, 'validateIP']],
                    'message' => Language::_('Centovacastpanelreseller.!error.ipaddress_valid', true)
                ]
            ],
			'hostname' => [
                'valid' => [
                    'rule' => [[$this, 'validateHostName']],
                    'message' => Language::_('Centovacastpanelreseller.!error.hostname_valid', true)
                ]
            ],
            'adminusername' => [
                'valid' => [
                    'rule' => 'isEmpty',
                    'negate' => true,
                    'message' => Language::_('Centovacastpanelreseller.!error.adminusername_valid', true)
                ]
            ],
            'adminapikey' => [
                'valid' => [
                    'rule' => 'isEmpty',
                    'negate' => true,
                    'message' => Language::_('Centovacastpanelreseller.!error.adminapikey_valid', true)
                ],
                'valid_connection' => [
                    'rule' => [
                    ...
                    'message' => '[Validate Connection] '.Language::_('Centovacastpanelreseller.!error.api.internal', true)
                ]
            ]
        ];
        ...
        $rules = [
			...
			'meta[package]' => [
                'valid' => [
                    'rule' => 'isEmpty',
                    'negate' => true,
                    'message' => Language::_('Centovacastpanelreseller.!error.meta[package].valid', true)
                ]
            ],
			*/
        ];
